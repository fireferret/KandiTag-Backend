...
GridJS.prototype.createWriteStream = function(filename, opts) {
	...
	var self = this;
	var ongridstore = thunky(function(callback) {
		self._open(filename, opts.flags || 'w', opts, callback);
	});
...
GridJS.prototype.unlink = function(filename, options, callback) {
	var self = this;
	if (!callback) callback = noop;
	this._init(function(err, db) {
		if (err) return callback(err);
		self._proto.unlink(db, filename, options, callback);
	});
};
